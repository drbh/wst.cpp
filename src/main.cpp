...
std::tuple<vec2D, vec1D> load()
{

    uint8_t buf[784];
    vec1D digit;

    auto fin = std::ifstream("../data/t10k-images-idx3-ubyte", std::ios::binary);
    if (!fin)
    {
...
int main()
{
    vec2D signal;
    vec1D digit;
    std::tie(signal, digit) = load();

    std::vector<outputs> scattering_output = scatter(signal, digit);

    vec2D coefficients;
    vec1D intercept;

    std::ifstream fin("../data/model.txt");
    if (!fin)
    {
